...
ルール:
- 1行目: 受信メールの差出人名から相手の名前を抽出し「〇〇様」とする
  - 名字だけ分かれば「山田様」、フルネームなら「山田太郎様」
  - 会社名やメールアドレスしか分からない場合は「ご担当者様」
- 2行目: 空行
- 3行目: 「お世話になっております。」（過去のやり取りで別のあいさつを使っていればそれに合わせる）
- 本文との間に空行
- 末尾の署名は必ず「博多ステラ矯正歯科　【名前】」とする（「【名前】」はそのまま記載、後で人間が埋める）
- 署名の直前にも空行を入れる
"""
...
def generate_reply(platform, incoming_message, past_exchanges):
    ...
    platform_extra = GMAIL_FORMAT_INSTRUCTION if platform == "Gmail" else ""
